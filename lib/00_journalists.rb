...
puts "Combien y a-t-il de journalistes?"
puts journalists.size
...
puts journalists.to_s.scan(/_/).count

puts '-' * 10

puts "ordre alphabétique"
puts journalists.sort_by {|k|[k]}

puts '-' * 10

puts "Quels sont les 50 handles les plus courts de ce array ?"
puts journalists.min_by(50) { |key| key.length }

puts '-' * 10

puts "Quelle est la position dans l'array de la personne @epenser ?"
puts journalists.to_a.index {|key,| key == "@epenser"}
